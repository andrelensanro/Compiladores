...
int analizadorLexico(vector<reg> d, char **a, string &lexema){
    ...
    while(*caracterActual!='\0'){
        ...
        cout<<"Estado actual: "<<estadoActualAFD<<endl;
        cout<<"Caracter actual: "<<*caracterActual<<endl;
        cout<<"Inilexema inicial: "<<*inilexema<<endl;
        ...
        cout<<"buscamos transicion de : "<<estadoActualAFD<<" con "<<*caracterActual<<endl;
        cout<<"Estado de aceptacion antes: "<<ac<<endl;
        int destino = findTransition(d, estadoActualAFD, *caracterActual, pToken, ac);
        cout<<"Destino: "<<destino<<endl;
        cout<<"Estado de aceptacion despues: "<<ac<<endl;
        //destino = state.cp_destino;
        if(destino==-1){
            cout<<"---------------- el destino es -1\n";
            if(!ac){
                cout<<"---------------- no hubo estado de aceptacion al buscar la transicion\n";
                inilexema++;
                *a = inilexema;
                return -1; // no hay estado de aceptacion y retorna eso
            }else{
                cout<<"---------------- si hubo estado de aceptacion al buscar la transicion\n";
                /*ADD*/

                //lexema = ""; // porque solo se agrego el ultimo, el que llego a estado final
                char *aux_i = inilexema;
                char *aux_f = finLexema;
                cout<<"inilexema "<<*aux_i<<endl;  
                ...
                cout<<"carcater actual cuando al encontrar la transicion me lleva a un estado de aceptacion\n";
            ...
            if(pToken!=0){ // es un estado final
                cout<<"como es estado final se agrega al lexema el caracter\n";
...
void lecturaAFD(vector<reg> &delta){
    ...
    ifstream f;
    reg registro;
    f.open("clasesLexicasCalculadoraCientifica.txt");
    if (f.is_open()){
        cout<<"se pudo abrir\n";
        while (!f.eof()){
...
int main(){
    string s = "2.89*sin(20-43.2/8)+cos(tan(2.5-9.7)+4)";
    callLexic(s);
}
