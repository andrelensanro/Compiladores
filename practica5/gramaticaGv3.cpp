...
vector<int> nodo2int(vector<nodo> v, map<string, int> & terminales, map<string, int> & noterminales){
    vector<int> filas;
    for(nodo n: v){
        if(terminales.count(n.simb))//existe en terminales
            filas.
...
void generartablaLL1(map<string, int> &noterminales, map<string, int> &terminales, vector<vector<pair<vector<int>, int>>> &tablall1, vector<izquierdo> &arrReglas){
    ...
    for(izquierdo iz: arrReglas){
        ...
        for(string str: firstR){
            string aux = nodo2int(iz.regla, terminales, noterminales);
            if(str == "$"){
...
int main(){
    ...
    if (f.is_open()){
        ...
        /*GENERAR TABLA LL1*/
        vector<vector<pair<vector<int>, int>>> tablaLL1(symbolNotTerminal.size()+terminales.size()+1, vector<pair<string, int>>(terminales.size()+1));
